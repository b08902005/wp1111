...
const useWordle = (solution) => {
    ...
    const handleEnter = () => {
        ...
        let solutionLetterSet = solution.split('');
        // console.log(solutionLetterSet)
        let newGuess = guesses.slice(0);
        let checkSet = [false, false, false, false, false];
        // console.log(newGuess);
        newGuess[turn] = new Array(5);
        for (let i = 0; i < 5; i++) {
        ...
        // console.log(newGuess);
        for (let i = 0; i < 5; i++) {
            if (curGuess[i] === solutionLetterSet[i]) {
                newGuess[turn][i]['color'] = 'green';
                solutionLetterSet[i] = '';
                checkSet[i] = true;
            }
        }
        for (let i = 0; i < 5; i++) {
            if (!checkSet[i]) {
                for (let j = 0; j < 5; j++) {
                    if (curGuess[i] === solutionLetterSet[j]) {
                        newGuess[turn][i]['color'] = 'yellow';
                        solutionLetterSet[j] = '';
                        checkSet[i] = true;
                        break;
                    }
        ...
        for (let i = 0; i < 5; i++) {
            if (newGuess[turn][i]['color'] === 'green') {
                setUsedChars((prev) => {
                    prev[`${newGuess[turn][i]['char']}`] = 'green';
                    return prev;
                })
            }
            else if (newGuess[turn][i]['color'] === 'yellow') {
                if (!usedChars[`${newGuess[turn][i]['char']}`] || usedChars[`${newGuess[turn][i]['char']}`] === 'grey') {
                    setUsedChars((prev) => {
                        prev[`${newGuess[turn][i]['char']}`] = 'yellow';
                        return prev;
                    })
                }
            }
