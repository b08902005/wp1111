...
exports.GetSearch = async (req, res) => {
    ...
    // console.log(priceFilternew);
    if (mealFilter) {
        if (typeFilter) {
            tagFilter = [...mealFilter, ...typeFilter]
        }
        tagFilter = [...mealFilter]
    } else if (typeFilter) {
        tagFilter = [...typeFilter]
    }
    let allFilter = {}
    if (priceFilternew){
        allFilter.price = { $in: priceFilternew };
    }
    if (tagFilter){
        allFilter.tag = { $all: tagFilter }
    }
    console.log(allFilter)
    const info = await Info.find(
        {
        price: { $in: priceFilternew },
        tag: { $all: tagFilter }
    }
    ).exec()
    // console.log(info);
